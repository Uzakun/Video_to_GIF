...
def generate_gifs_route():
    ...
    try:
        ...
        # 2. Download Video using yt-dlp with more robust options
        ...
        # This checks for other possible extensions like .mkv
        if not os.path.exists(video_path):
            for ext in ['mkv', 'webm']:
                 alt_path = video_path_template.replace('.%(ext)s', f'.{ext}')
                 if os.path.exists(alt_path):
                     video_path = alt_path
                     break
        
        if not os.path.exists(video_path):
             print("DEBUG: Video download failed, file not found after checking extensions.")
        ...
        if not transcript:
            ...
            return jsonify({'error': 'Could not fetch transcript for this video. It might be disabled.'}), 400
        ...
        # Don't print the full traceback for this specific error
        print(f"DEBUG: yt-dlp failed with message: {e}")
        print("-----------------------------")
